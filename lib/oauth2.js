...
p.request = function (options, params, accessToken, multi, callback) {
  ...
  req = https.request(options, function (res) {
    ...
    res.on('end', function () {
      ...
      try {
        /* because weibo sends us partial responses, crashing node */
        var data = JSON.parse(result);
      } catch (Error) {
          console.log('Invalid JSON');
          res.statusCode = 206; //partial content header
          var data = {"Error": "Invalid response"};
      }


      if (res.statusCode != 200) { //didnt understand why redirects were also sent back, so just checked if it wasnt a 200 response
